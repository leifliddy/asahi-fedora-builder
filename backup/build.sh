...
# if the kernel is installed via mkosi -- the following error will occur
# note: installing asahi-platform-metapackage will cause the kernel to be installed
#
#>>> Running %posttrans scriptlet: kernel-16k-core-0:6.17.9-400.asahi.fc42.aarch64
#>>> Non-critical error in %posttrans scriptlet: kernel-16k-core-0:6.17.9-400.asahi.fc42.aarch64
#>>> Scriptlet output:
...
make_image() {
    ...
    echo -e '\n### Installing additional packages'install_kernel_in_rootfs
    arch-chroot $mnt_image dnf install -y kernel-16k-core kernel-16k-modules-extra asahi-platform-metapackage
